...
const BILLING_PAIN = [
  ...
  /\b(vercel|heroku|firebase|aws|gcp|azure).{0,20}(expensive|bill|cost|pricing)/i,
];
...
const MIGRATION_INTEREST = [
  ...
  /\balternative\s*to\s*(vercel|heroku|firebase|aws|gcp|azure|netlify|render)/i,
  /\b(self[- ]host|coolify|dokku|kamal|hetzner).{0,20}(instead|replace|alternative|migrate)/i,
  /\b(looking|searching|considering|evaluating).{0,20}(alternative|replacement|migration|new\s*host)/i,
  /\b(moved?|switch)\s*to\s*(cloudflare|hetzner|vps|self[- ]host|coolify)/i,
  /\bhow\s*(do|to|should).{0,15}(migrate|move|switch|deploy).{0,15}(off|from|away)/i,
];

const PLATFORM_COMPLAINT = [
  /\b(heroku|vercel|firebase|aws|netlify).{0,20}(stagnant|maintenance mode|no new features|sunset|deprecated|killed|removed)/i,
  /\b(vendor|platform)\s*(lock[- ]?in|dependency)/i,
  /\b(support|reliability|uptime).{0,15}(terrible|awful|bad|poor|worst|nonexistent)/i,
  /\b(security|breach|incident|compromised).{0,15}(vercel|heroku|firebase)/i,
  /\b(forced|unexpected)\s*(migration|upgrade|change)/i,
  /\bfree\s*tier.{0,15}(removed|killed|eliminated|gone)/i,
];
